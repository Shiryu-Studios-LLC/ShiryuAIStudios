...
import { IShiryuAiService, IShiryuAiResponse, IShiryuModelConfig, IShiryuModelInfo } from './shiryuAiService.js';
...
export class ShiryuAiService extends Disposable implements IShiryuAiService {
	...
	get isAvailable(): boolean {
		return this._model !== undefined && this._context !== undefined;
	}
	...
	async loadModel(config: IShiryuModelConfig): Promise<void> {
		...
		try {
			...
			this._model = await (this._llama as any).loadModel({
